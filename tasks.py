...
@task(
    help={
        ...
        'browser': 'Whether to open the application in a \
        browser window. Default: false',
...
@task(
    help={
        ...
        'database-path': 'Path to sqlite database \
         (absolute path recommended). Leave empty for default',
        'address': 'Address to use. Default: localhost',
        'port': 'Port to use. Default: 8000',
        'browser': 'Whether to open the application\
         in a browser window. Default: false',
        'start-server': 'Whether to start \
         the development server. Default: true'})
...
def create_settings(
    ...
    secret_key = get_random_string(key_length, 'abcdefghijklmnopqrstuvwxyz012\
    3456789!@#$%^&*(-_=+)')
    ...
    settings_content = settings_content.format(dbname=dbname,
                                               ...
                                               siteurl=url,
                                               media_folder_path=media_folder_path)  # noqa
...
def config_location(context):
    ...
    print('* database path: {0}'.format(get_user_data_path('wger', 'database.\
          sqlite')))
