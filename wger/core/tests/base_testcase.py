...
class WorkoutManagerTestCase(BaseTestCase, TestCase):
    ...
    def compare_fields(self, field, value):
        ...
        # decimal, convert
            ...
            self.assertEqual(field.quantize(TWOPLACES), decimal.Decimal(value).
                             quantize(TWOPLACES))
...
class WorkoutManagerDeleteTestCase(WorkoutManagerTestCase):
    ...
    def delete_object(self, fail=False):
        ...
        if fail:
            ...
            # additional redirect The page we are redirected to doesn't trigger
            # an error response = self.client.get(response['Location'])
            # self.assertEqual(response.status_code, 200)
...
class WorkoutManagerAddTestCase(WorkoutManagerTestCase):
    """
    Tests adding an object as an authorized user, a
    different one and a logged out one.
    """
...
class WorkoutManagerAccessTestCase(WorkoutManagerTestCase):
    ...
    an unauthorized one and a logged out one.
    """
    ...
    def access(self, fail=True):
        ...
        if fail:
            ...
            # TODO: the redirection page might not have a language prefix
            # (e.g. /user/login instead of /en/user/login) so there is an
            # additional redirect if response.status_code == 302:
            # The page we are redirected to doesn't trigger an error
            # response = self.client.get(response['Location'])
            # self.assertEqual(response.status_code, 200)
