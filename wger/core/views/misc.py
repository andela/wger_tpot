...
def demo_entries(request):
    ...
    if (((not request.user.is_authenticated() or request.user.userprofile.is_temporary)  # noqa
        ...
        messages.success(request, _('We have created sample workout, workout schedules,\
                                      weight '
                                    'logs, (body) weight and nutrition plan \
                                    entries so you can '
                                    'better see what  this site can do. Feel\
                                     free to edit or '
                                    'delete them!'))
    return HttpResponseRedirect(reverse('core:dashboard'))
...
def dashboard(request):
    ...
    (current_workout, schedule) = Schedule.objects.get_current_workout(request.user)  # noqa
    ...
    try:
        plan = NutritionPlan.objects.filter(user=request.user).latest('creation_date')  # noqa
...
class FeedbackClass(FormView):
    ...
    def get_form_class(self):
        ...
        """
        if self.request.user.is_anonymous() or self.request.user.userprofile.is_temporary:  # noqa
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, _('Your feedback was successfully \
                         sent. Thank you!'))
