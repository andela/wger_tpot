...
class UserProfile(models.Model):
    ...
    show_comments = models.BooleanField(verbose_name=_('Show exercise \
                                                       comments'),
                                        help_text=_('Check to show exercise \
                                                     comments on the '
                                                    'workout view'),
                                        default=True)
    """
    ...
    # (obviously this is only meaningful if the user has a language other than
    #  english)
...
the search difficult."""),
    ...
    workout_duration = IntegerField(verbose_name=_('Default duration of \
                                    ...
                                    help_text=_('Default duration in weeks\
                                                of workouts not '
                                                'in a schedule. Used for email workout '  # noqa
                                                'reminders.'),
                                    default=12,
                                    validators=[MinValueValidator(1),
                                    MaxValueValidator(30)])
    ...
    notification_language = models.ForeignKey(Language,
                                              verbose_name=_('Notification\
                                               language'),
                                              help_text=_('Language to use when\
                                               sending you email '
                                                          'notifications, e.g. \
                                                          email reminders for '
                                                          'workouts. This does\
    ...
    timer_active = models.BooleanField(verbose_name=_('Use pauses in\
     workout timer'),
                                       help_text=_('Check to activate timer \
                                       pauses between '
                                                   'exercises.'),
                                       default=True)
    """
    ...
    ro_access = models.BooleanField(verbose_name=_('Allow external access'),
                                    help_text=_('Allow external users to access\
                                     your workouts and '
                                                'logs in a read-only mode. You \
                                                need to set this '
                                                'before you can share links \
                                                e.g. to social media.'),
                                    default=False)
    """Allow anonymous read-only access"""

    num_days_weight_reminder = models.IntegerField(verbose_name=_('Automatic\
     reminders for weight '
                                                                  'entries'),
                                                   ...
                                                   after the last '
                                                               ...
                                                               (enter 0 to '
                                                               'deactivate)'),
                                                   validators=[MinValueValidator  # noqa
                                                               (0),
                                                               MaxValueValidator(30)],  # noqa
                                                   default=0)
    ...
    def clean(self):
        ...
        if ((self.sleep_hours and self.freetime_hours and self.work_hours)
                and (self.sleep_hours + self.freetime_hours + self.work_hours) > 24):  # noqa
    ...
    def calculate_bmi(self):
        ...
        weight = self.weight if self.use_metric else AbstractWeight(self.weight,  # noqa
                                                                    'lb').kg
...
class License(models.Model):
    ...
    full_name = models.CharField(max_length=60,
                                 ...
                                 help_text=_('If a license has been localized, \
                                 e.g. the Creative '
                                             'Commons licenses for the \
                                             different countries, add '
                                             'them as separate entries here.'))
    """Full name"""

    short_name = models.CharField(max_length=15,
                                  verbose_name=_('Short name, e.g.\
                                   CC-BY-SA 3'))
    """Short name, e.g. CC-BY-SA 3"""

    url = models.URLField(verbose_name=_('Link'),
                          help_text=_('Link to license text or other \
                          information'),
                          blank=True,
                          null=True)
